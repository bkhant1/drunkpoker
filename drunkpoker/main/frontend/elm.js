...
var $author$project$Main$initFromBeforeSit = _Utils_Tuple2(
	{
		...
		playerName: 'Hello',
		playerState: $author$project$Main$Sitting(1)
	},
	$elm$core$Platform$Cmd$none);
var $author$project$Main$init = function (flags) {
	return $author$project$Main$initFromBeforeSit;
};
...
var $author$project$Main$GameState = function (seats) {
	return {seats: seats};
};
...
var $author$project$Main$Player = F3(
	function (name, state, cards) {
		return {cards: cards, name: name, state: state};
	});
...
var $author$project$Main$gameStateFromJsonState = function (jsonState) {
	var stringStateToUiState = F2(
		function (stringState, seatNumber) {
			switch (stringState) {
				...
				case 'MY_TURN':
					return $author$project$Main$Playing(seatNumber);
				case 'IN_GAME':
					return $author$project$Main$Playing(seatNumber);
				case 'FOLDED':
					return $author$project$Main$Playing(seatNumber);
	...
	return $author$project$Main$GameState(
		$elm$core$Dict$fromList(
...
var $author$project$Main$JsonPlayer = F3(
	function (name, state, cards) {
		return {cards: cards, name: name, state: state};
	});
var $author$project$Main$JsonState = F2(
	function (seats, players) {
		return {players: players, seats: seats};
	});
...
var $author$project$Main$jsonStateDecoder = A3(
	$elm$json$Json$Decode$map2,
		...
		switch (msg.$) {
			case 'Recv':
				...
				if (_v1.$ === 'Ok') {
					...
					return A2(
						$elm$core$Debug$log,
						'I am fucking here bitch ' + $elm$core$Debug$toString(error),
			...
			case 'EnterName':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							playerState: $author$project$Main$Sitting(seatNumber)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Sit':
				var seatNumber = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							playerState: $author$project$Main$Playing(seatNumber)
						}),
					$elm$http$Http$post(
						{
...
var $author$project$Main$renderPlayer = F3(
	function (player, position, mySeatNumber) {
	...
	function (model, position, seatNumber) {
		...
		if (_v0.$ === 'Nothing') {
			...
			if (_v0.a.$ === 'Nothing') {
				...
				var player = _v0.a.a;
				return A3($author$project$Main$renderPlayer, player, position, seatNumber);
			}
		}
	});
var $author$project$Main$playerSits = function (model) {
	return _List_fromArray(
		[
			...
			model,
			{pctLeft: 45, pctTop: 17},
			2),
			A3(
			$author$project$Main$playerSit,
			model,
			{pctLeft: 10, pctTop: 31},
			1),
			A3(
			$author$project$Main$playerSit,
			model,
			{pctLeft: 80, pctTop: 31},
			3),
			A3(
			$author$project$Main$playerSit,
			model,
			{pctLeft: 10, pctTop: 63},
			6),
			A3(
			$author$project$Main$playerSit,
			model,
			{pctLeft: 80, pctTop: 63},
			4),
			A3(
			$author$project$Main$playerSit,
			model,
			{pctLeft: 45, pctTop: 75},
			5)
		]);
};
